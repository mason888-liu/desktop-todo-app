...
const DJG_URL = 'https://tfxportal.dgjapp.com/FFAccount?message=%e7%a5%96%e5%9b%bd%e6%94%be%e5%bf%83%e8%9b%8b&sid=175409186&pt=TouTiao&to';
const OUTPUT = 'E:/first_cc/djg_selectors.txt';

let stepNum = 0;
let clickCount = 0;
let inputCount = 0;

// This script auto-injects on every navigation via addInitScript
...
(async () => {
  fs.writeFileSync(OUTPUT, `=== DJG 店管家选择器记录 ===\n${new Date().toISOString()}\n\n`);
  ...
  // -- Expose Node.js handlers BEFORE addInitScript --
  ...
  // addInitScript persists across all navigations — no need to re-inject
  await page.addInitScript(injectRecorder);

  // Log page navigations (load event fires once per main frame)
  page.on('load', async () => {
    stepNum++;
    const url = page.url();
  ...
  await page.goto(DJG_URL, { waitUntil: 'domcontentloaded' });
